...
import { RowContext } from 'components/table/admin/requestsColumn'

function PaginationRequestTable(props) {
  const [setLoader, bindLoader, closeLoader] = Loader('')
  const { setSelectedRowId } = useContext(RowContext);
  ...
  const handleApprove=async(id)=>{
    ...
    const data = axios.post('http://localhost:3008/api/admin/spRequest',data)
  }
...
export default PaginationRequestTable
